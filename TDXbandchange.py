...
def checkinlist(list,code1):
    for i in list:
        ...
        #print(i,codenum,codename)
        if code1 in i:
            return codenum,codename


            #通达信竞价文件读取 只读取最后两行,并进行计算
def readTDXdata(filename,codenum,codename):
    listdata=[]
...
def tdxbandchange():
    ...
    newfilelist=[]       #板块文件列表
    valuelist=[]
    for file in filelist:    #只需要带有板块文件
        codenum=date1 = re.search(r'\d+.dat$', file).group()[0:6]
        codem,codename=checkinlist(bandlist, codenum)
        if codem and codename:
            #更新列表
            newfilelist.append(sdir+'\\'+file)
        else:
            continue

    for newfile in newfilelist:
        try:
            codenum1,codename1,value=readTDXdata(newfile,codenum,codename)
            valuelist.append(codenum1+':'+codename1+':'+value)
