    ...
    class DummyAkShareUnit:
        def _create_dummy_df(self, columns, num_rows=5, prefix="dummy"):
            ...
            for col_info in columns:
                ...
                if col_name == '代码' or col_name == '股票代码': data[col_name] = [f'{prefix}{i:03d}' for i in range(1, num_rows + 1)]
                elif col_name == '名称' or col_name == '股票名称' or col_name == '板块名称': data[col_name] = [f'Dummy {prefix.capitalize()} {i}' for i in range(1, num_rows + 1)]
                ...
                elif '主力净流入-净额' in col_name: data[col_name] = [50000 * i for i in range(1,num_rows+1)] # Example values
                elif '主力净流入-净占比' in col_name: data[col_name] = [0.1 * i for i in range(1,num_rows+1)] # Example percentages
                ...
                elif col_name == '上榜次数': data[col_name] = [num_rows - i for i in range(num_rows)]
                elif '累积购买额' in col_name : data[col_name] = [1E6 * i for i in range(1,num_rows+1)]
                elif '日期' in col_name : data[col_name] = [(pd.Timestamp.now() - pd.Timedelta(days=i)).strftime('%Y-%m-%d') for i in range(num_rows)]
                ...
                elif col_name == '当日资金流入': data[col_name] = [1E7 * (num_rows-i) for i in range(num_rows)]
                elif col_name == '持股市值': data[col_name] = [1E8 * (i+1) for i in range(num_rows)]
        ...
        def get_large_orders_data_dummy(self): return self._create_dummy_df(['代码', '名称', '最新价', '涨跌幅', '主力净流入-净额', '主力净流入-净占比', '超大单净流入-净额'], 50, "LO")
        ...
        def get_cls_news_dummy(self): return self._create_dummy_df(['时间', '内容', '快讯ID', '资讯类型'], 25, "CLS")
        def get_newly_highlighted_stocks_dummy(self): return self._create_dummy_df(['股票代码', '股票名称', '上榜次数', '累积购买额', '累积卖出额', '净额'], 20, "LHB")
        def get_market_fundflow_overview_dummy(self): return self._create_dummy_df(['日期', '上证-主力净流入-净额', '深证-主力净流入-净额', '创业板-主力净流入-净额'], 10, "MarketFlow")

        def get_northbound_net_flow_dummy(self): return self._create_dummy_df(['日期', '当日资金流入', '当日余额', '历史资金累计流入'], 30, "NBFlow")
        def get_southbound_net_flow_dummy(self): return self._create_dummy_df(['日期', '当日资金流入', '当日余额', '历史资金累计流入'], 30, "SBFlow")
        def get_northbound_top_holdings_dummy(self): return self._create_dummy_df(['代码', '名称', '最新价', '持股市值', '持股数量占A股百分比'], 20, "NBHold")
        def get_hgt_top_holdings_dummy(self): return self._create_dummy_df(['代码', '名称', '最新价', '持股市值'], 20, "HGTHold")
        def get_sgt_top_holdings_dummy(self): return self._create_dummy_df(['代码', '名称', '最新价', '持股市值'], 20, "SGTHold")
...
def _fetch_data_with_fallback(ak_method_name_custom_or_direct_func, page, per_page, *args, **kwargs):
    ...
    try:
        ...
        if per_page is None or per_page == 0: # No pagination, return full DataFrame
            return df, len(df) if df is not None else 0, 1, 1
        ...
        target_dummy_method_name = dummy_method_name
        if is_method_name and hasattr(DummyAkShareUnit(), ak_method_name_custom_or_direct_func):
             target_dummy_method_name = ak_method_name_custom_or_direct_func

        if hasattr(DummyAkShareUnit(), target_dummy_method_name):
             df_dummy = getattr(DummyAkShareUnit(), target_dummy_method_name)()
        else:
            print(f"Could not find specific dummy method {target_dummy_method_name}, returning empty DataFrame.")
            df_dummy = pd.DataFrame()

        if per_page is None or per_page == 0:
            return df_dummy, len(df_dummy) if df_dummy is not None else 0, 1, 1
...
def get_leading_stocks_data():
    ...
    except requests.exceptions.RequestException as e: print(f"Net err for leading stocks: {e}"); return dummy_return
    except Exception as e: print(f"Proc err for leading stocks: {e}"); return dummy_return
...
def get_fund_data_selected_stocks(indicator="今日", min_main_force_percent=None, page=1, per_page=20):
    try:
        # Use lambda for ak_direct_func to pass the indicator argument
        df, total_items_unfiltered, total_pages_unfiltered, _ = _fetch_data_with_fallback(
            'get_large_orders_data_dummy', # Dummy method name if direct call fails
            lambda: ak.stock_individual_fund_flow_rank(indicator=indicator),
            1, # Fetch all data first for filtering
            0, # per_page = 0 to fetch all
            indicator=indicator # This kwarg is for the lambda
        )

        if df is not None and not df.empty and min_main_force_percent is not None:
            # Ensure the column name matches what akshare returns. It's often '主力净流入-净占比'.
            # The values in this column are percentages (e.g., 5.5 for 5.5%).
            if '主力净流入-净占比' in df.columns:
                # Convert to numeric, coercing errors. Original data might have '%' symbols or be strings.
                df['主力净流入-净占比'] = pd.to_numeric(df['主力净流入-净占比'], errors='coerce')
                df_filtered = df[df['主力净流入-净占比'] >= min_main_force_percent].copy() # Use .copy() to avoid SettingWithCopyWarning
            else:
                print("Warning: '主力净流入-净占比' column not found for filtering.")
                df_filtered = df # Return unfiltered if column is missing
        else:
            df_filtered = df

        # Now paginate the (potentially filtered) DataFrame
        paginated_df, total_items, total_pages = _paginate_dataframe(df_filtered, page, per_page)
        return paginated_df, total_items, total_pages, page

    except Exception as e:
        print(f"Error in get_fund_data_selected_stocks: {e}. Using dummy data as fallback.")
        df_dummy = DummyAkShareUnit().get_large_orders_data_dummy() # Use relevant dummy
        if min_main_force_percent is not None and '主力净流入-净占比' in df_dummy.columns:
             df_dummy['主力净流入-净占比'] = pd.to_numeric(df_dummy['主力净流入-净占比'], errors='coerce')
             df_dummy = df_dummy[df_dummy['主力净流入-净占比'] >= min_main_force_percent].copy()
...
# --- News ---
def get_cls_news(page=1, per_page=20): return _fetch_data_with_fallback('get_cls_news_dummy', ak.cls_telegraph_news, page, per_page)

# --- Newly Highlighted & Market Fundflow ---
def get_newly_highlighted_stocks(page=1, per_page=20, recent_day="5"): return _fetch_data_with_fallback('get_newly_highlighted_stocks_dummy', lambda: ak.stock_sina_lhb_ggtj(recent_day=recent_day), page, per_page)
def get_market_fundflow_overview(page=1, per_page=50): return _fetch_data_with_fallback('get_market_fundflow_overview_dummy', ak.stock_market_fund_flow, page, per_page)

# --- North/South Bound Flows ---
def get_northbound_net_flow(page=1, per_page=0): return _fetch_data_with_fallback('get_northbound_net_flow_dummy', lambda: ak.stock_em_hsgt_north_net_flow_in(indicator="北上"), page, per_page)
def get_southbound_net_flow(page=1, per_page=0): return _fetch_data_with_fallback('get_southbound_net_flow_dummy', lambda: ak.stock_em_hsgt_south_net_flow_in(indicator="南下"), page, per_page)
def get_northbound_top_holdings(page=1, per_page=20, indicator="今日排行"): return _fetch_data_with_fallback('get_northbound_top_holdings_dummy', lambda: ak.stock_em_hsgt_hold_stock(market="北向", indicator=indicator), page, per_page)
def get_hgt_top_holdings(page=1, per_page=20, indicator="今日排行"): return _fetch_data_with_fallback('get_hgt_top_holdings_dummy', lambda: ak.stock_em_hsgt_hold_stock(market="沪股通", indicator=indicator), page, per_page)
def get_sgt_top_holdings(page=1, per_page=20, indicator="今日排行"): return _fetch_data_with_fallback('get_sgt_top_holdings_dummy', lambda: ak.stock_em_hsgt_hold_stock(market="深股通", indicator=indicator), page, per_page)
