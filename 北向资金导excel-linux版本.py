...
def result_to_htmlAExcel(all_Everydata,DateType):
    ...
    if DateType!=1:
        outfile='/opt/lampp/htdocs/北向资金_'+HdDate+'_'+str(DateType)+'.html'
        fw = open(outfile, 'w', encoding='gbk')
        print(s,file=fw)
        fw.close()

        with open('/opt/lampp/htdocs/北向资金_'+HdDate+'_'+str(DateType)+'.csv','w',encoding='gbk') as fcsv:
            fcsv.write(sb)
        fcsv.close()

        outfile='/opt/lampp/htdocs/北向资金'+'_'+str(DateType)+'.html'
        fw = open(outfile, 'w', encoding='gbk')
        print(s,file=fw)
        fw.close()

        outfile='/opt/lampp/htdocs/北向资金'+'_'+str(DateType)+'.csv'
        fw = open(outfile, 'w', encoding='gbk')
        print(sb,file=fw)
        fw.close()
    else:
        outfile = '/opt/lampp/htdocs/北向资金_' + HdDate + '.html'
        fw = open(outfile, 'w', encoding='gbk')
        print(s, file=fw)
        fw.close()

        with open('/opt/lampp/htdocs/北向资金_' + HdDate  + '.csv', 'w', encoding='gbk') as fcsv:
            fcsv.write(sb)
        fcsv.close()

        outfile = '/opt/lampp/htdocs/北向资金' + '.html'
        fw = open(outfile, 'w', encoding='gbk')
        print(s, file=fw)
        fw.close()

        outfile = '/opt/lampp/htdocs/北向资金' + '.csv'
        fw = open(outfile, 'w', encoding='gbk')
        print(sb, file=fw)
        fw.close()
    #print(tb)
